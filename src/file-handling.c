...
FILE **open_files(int argc, char **argv, int *num_input_files, int output_file_index) {
    int input_is_stdin = (argc == 1 && output_file_index == -1) || (argc == 3 && output_file_index != -1);
    if (input_is_stdin) {
        *num_input_files = 1;
    }
    else if (output_file_index == -1) {
    ...
    for (int i = 1, current_file = 0; i < argc; ++i) {
...
void print_date(FILE *out) {
    ...
    fprintf(out, "%d-%d-%d", current_time->tm_year, current_time->tm_mon + 1, current_time->tm_mday);
}

void print_time(FILE *out) {
    struct tm *current_time = localtime(time(NULL));
    fprintf(out, "%d:%d:%d", current_time->tm_hour, current_time->tm_min, current_time->tm_sec);
}
